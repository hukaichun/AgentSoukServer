...
async def receive_hello(websocket: WebSocket) -> dict[str, Any] | None:
    ...
    must be a prompt, well-formed `hello`. Returns it, or None after
    closing the socket — anything else before hello closes it (per
    docs/server-mode.md), so a caller only ever proceeds or returns."""
    try:
